SELECT prods_.virtuemart_product_id
 ...
 WHERE (
        prods_.virtuemart_product_id = (  
        SELECT MAX(prods0.virtuemart_product_id)
          FROM auc13_virtuemart_products prods0
         WHERE prods0.virtuemart_product_id < 551
           )     
        OR
        prods_.virtuemart_product_id = 551
        OR
        prods_.virtuemart_product_id = (  
        SELECT MIN(prods00.virtuemart_product_id)
          FROM auc13_virtuemart_products prods00
         WHERE prods00.virtuemart_product_id > 551
        )
       ) AND 
        prods_.virtuemart_product_id IN (
            SELECT 
            ...
            WHERE cat_cats.category_parent_id = (
                SELECT cat_cats1.category_parent_id
                 ...
                 WHERE cat_cats1.category_child_id = 6
              )
      )