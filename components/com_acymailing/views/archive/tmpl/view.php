...
<?php if($this->frontEndManagement AND ($this->config->get('frontend_modif',1) || ($this->mail->userid == $this->my->id)) AND ($this->config->get('frontend_modif_sent',1) || empty($this->mail->senddate))){ ?>
	...
	<?php }
		...
		if(JRequest::getString('tmpl') == 'component'){?>
			<a title="<?php echo JText::_( 'ACY_PRINT',true ); ?>" href="#" onclick="window.print();return false;"><?php echo $printimage; ?></a>
		<?php }else{ ?>
			<a title="<?php echo JText::_( 'ACY_PRINT',true ); ?>" href="<?php echo acymailing_completeLink($link,true); ?>" onclick="window.open(this.href,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no'); return false;" rel="nofollow"><?php echo $printimage; ?></a>
		<?php }	?>
		</td>
	<?php } ?>
		</tr></table>
