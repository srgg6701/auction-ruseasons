 ...
 * @subpackage
 */
function checkAuctionLayout() {
    return in_array(JRequest::getVar('layout'), getTopCatsLayouts());
}
/* 	MODIFIED END	 */
/**
 *
...
function virtuemartBuildRoute(&$query, $show = false) {
    ...
    switch ($view) {
        ...
        case 'category';
             ...
             */
            if ($query['virtuemart_category_id'] == '0' && (int) $query['Itemid']
            ) {
            ...
            if (isset($query['virtuemart_category_id'])) {
                ...
                if (isset($jmenu['virtuemart_category_id'][$query['virtuemart_category_id']])) {
                    ...
                    /**
                        $query[Itemid] изменяется здесь: */
                    //echo "<pre>"; var_dump($helper); echo "</pre>"; // die();
                    $categoryRoute = $helper->getCategoryRoute($query['virtuemart_category_id']);
                    //TEST:				
                    //setDebugTrace('\components\com_virtuemart\router.php','virtuemartBuildRoute::getCategoryRoute',__LINE__,'! isset($jmenu[\'virtuemart_category_id\'][ $query[\'virtuemart_category_id\']])','$query[\'virtuemart_category_id\'] = '.$query['virtuemart_category_id'],true);
                    //TEST
                ...
                //if ($query['virtuemart_category_id']!=='0')
            ...
            if (isset($query['order'])) {
                if ($query['order'] == 'DESC')
                    $segments[] = $helper->lang('orderDesc');
                unset($query['order']);
            }
            ...
            if ($start !== null && $limitstart !== null) {
                ...
                $segments[] = $helper->lang('results') . ',1-' . $limit; //limit change
        ...
        case 'productdetails';
            ...
            if (isset($jmenu['virtuemart_product_id'][$query['virtuemart_product_id']])) {
                //echo "<h1 style='color:red'>ISSET: ".$jmenu['virtuemart_product_id'][ $query['virtuemart_product_id'] ]."</div>";
                ...
                if (!empty($query['virtuemart_category_id'])) {
                    ...
                    if ($categoryRoute->itemId) {
                        /* 	MODIFIED START */
                        if (!checkAuctionLayout())
                        /* 	MODIFIED END	 */
                            $query['Itemid'] = $categoryRoute->itemId;

                        //echo "<h1 style='color:red'>IF, $query[virtuemart_category_id], $query[Itemid]</H1>";
                    }
                    else {
                        $query['Itemid'] = $jmenu['virtuemart'];
                    }
...
class vmrouterHelper {
    ...
    public static function getInstance(&$query = null) {
        ...
        if (!array_key_exists($instanceKey, self::$_instances)) {
            ...
            if (self::$limit === null) {
                ...
                if (isset($query['view']))
                    $view = $query['view'];
                self::$limit = $mainframe->getUserStateFromRequest(
                                                'com_virtuemart.' . $view . '.limit', 
        ...
        /* echo "<h2>instanceKey = $instanceKey<br>\$_instances[\$instanceKey]:</h2>";
          var_dump($_instances[$instanceKey]); */
    ...
    public function setLangs($instanceKey) {
        ...
        if (count($langs) > 1) {
            ...
            $this->vmlang = $this->langTag = VMLANG;
    ...
    public function getCategoryRouteNocache($virtuemart_category_id) {
        /**
            1. JRouterSite->build(index.php?option=com_virtuemart&view=category&virtuemart_category_id=21, )
                2. JRouter->build(index.php?option=com_virtuemart&view=category&virtuemart_category_id=21, )
                    3. JRouterSite->_buildSefRoute(JURI Object ([] => index.php?option=com_virtuemart&view=category&virtuemart_category_id=21,[] => ,[] => ,[] => ,[] => ,[] => ,[] => index.php,[] => ,[] => ,[] => Array ([option] => com_virtuemart,[view] => category,[virtuemart_category_id] => 21,[Itemid] => 125),[] => Array ()), )
                        4. virtuemartBuildRoute(Array ([option] => com_virtuemart,[virtuemart_category_id] => 21,[Itemid] => 125), 
                           called from includes/router.php)
        ...
        if (!array_key_exists($virtuemart_category_id . $this->vmlang, self::$_catRoute)) {
            ...
            if ($ismenu == false) {
                if ($this->use_id)
                    $category->route = $virtuemart_category_id . '/';
                ...
                if ($menuCatid == 0 && $this->menu['virtuemart'])
                    $category->itemId = $this->menu['virtuemart'];
            }
            self::$_catRoute[$virtuemart_category_id . $this->vmlang] = $category;
        }

        //TEST:	//setDebugTrace('components/com_virtuemart/router.php','virtuemartParseRoute::getCategoryRouteNocache',__LINE__,'','argument: virtuemart_category_id = '.$virtuemart_category_id,false);
        //TEST				
    ...
    public function getCategoryNames($virtuemart_category_id, $catMenuId = 0) {
        ...
        foreach ($parents_id as $id) {
            if (!isset($categoryNamesCache[$id])) {
                ...
                $cslug = $db->loadResult();
                //TEST:				
                //setDebugTrace('components/com_virtuemart/router.php', 'getCategoryNames::getCategoryNames->$db->loadResult()', __LINE__, '', '$cslug = '.$cslug, false); 
                //TEST				
        ...
        //echo "<div class=''>getCategoryNames</div>";
        //var_dump($strings);
        //echo "<div class=''>virtuemart_category_id= ".$virtuemart_category_id."</div>";
        if (checkAuctionLayout())
            unset($strings[0]);

        //TEST:					//setDebugTrace('components/com_virtuemart/router.php','getCategoryNames::getCategoryNames',__LINE__,'','virtuemart_category_id = '.$virtuemart_category_id.'<br>strings = '.$strs,'backtrace true');
        //TEST
    ...
    public function getCategoryRecurse($virtuemart_category_id, $catMenuId, $first = true) {
        ...
        //TEST:					//setDebugTrace('/components/com_virtuemart/router.php','JFactory::getDBO::loadObject',__LINE__,'','',false);
        //TEST
        ...
        //TEST:				
        //setDebugTrace('/components/com_virtuemart/router.php','vmrouterHelper::getCategoryRecurse',__LINE__,'','Рекурсивная функция','backtrace true');
        //TEST
